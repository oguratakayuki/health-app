...
import { IngredientNutrient } from '@/domain/entities/IngredientNutrient';
...
export class IngredientNutrientRepository implements IIngredientNutrientRepository {
  ...
  async findById(id: bigint): Promise<IngredientNutrient | null> {
    const ingredientNutrient = await this.prismaClient.ingredientNutrient.findUnique({
      where: { id },
      include: {
        ingredient: true,
        nutrient: true,
  ...
  async update(id: bigint, data: Partial<IngredientNutrient>): Promise<IngredientNutrient> {
    throw new Error('Method not implemented.');
  }
...
private mapToDomain(prismaItem: any): IngredientNutrientWithRelations {
  ...
  if (prismaItem.ingredient && prismaItem.ingredient.id) {
    ...
    console.warn(`IngredientNutrient ${prismaItem.id}: ingredient is null or missing id`);
  }
  ...
  if (prismaItem.nutrient && prismaItem.nutrient.id) {
    ...
    console.warn(`IngredientNutrient ${prismaItem.id}: nutrient is null or missing id`);
  }

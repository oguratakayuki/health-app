...
describe('DishService', () => {
  ...
  describe('getDish', () => {
    ...
    it('should throw error when repository fails', async () => {
      ...
      // 実行 & 検証
      await expect(dishService.getDish('1')).rejects.toThrow('Failed to get dish: Database error');
    });
  });
  ...
  describe('createDish', () => {
    ...
    it('should throw error when name is empty', async () => {
      ...
      // 実行 & 検証
      await expect(dishService.createDish(createInput)).rejects.toThrow('Dish name is required');
  ...
  describe('updateDish', () => {
    ...
    it('should throw error when dish not found', async () => {
      ...
      // 実行 & 検証
      await expect(dishService.updateDish('999', updateInput))
        .rejects.toThrow('Dish with id 999 not found');
    ...
    it('should throw error when new name is empty', async () => {
      ...
      // 実行 & 検証
      await expect(dishService.updateDish('1', updateInput))
        .rejects.toThrow('Dish name cannot be empty');
      expect(mockDishRepository.update).not.toHaveBeenCalled();
    });
  });
  ...
  describe('deleteDish', () => {
    ...
    it('should throw error when dish not found', async () => {
      ...
      // 実行 & 検証
      await expect(dishService.deleteDish('999'))
        .rejects.toThrow('Dish with id 999 not found');
  ...
  describe('searchDishesByName', () => {
    it('should search dishes by name', async () => {
      ...
      const mockDishes: Dish[] = [
        { id: '1', name: 'Chicken Curry', createdAt: new Date(), updatedAt: new Date() },
        { id: '2', name: 'Chicken Salad', createdAt: new Date(), updatedAt: new Date() }
      ];
    ...
    it('should return all dishes when search term is empty', async () => {
      // 準備
      const mockDishes: Dish[] = [
        { id: '1', name: 'Dish 1', createdAt: new Date(), updatedAt: new Date() },
        { id: '2', name: 'Dish 2', createdAt: new Date(), updatedAt: new Date() }
      ];

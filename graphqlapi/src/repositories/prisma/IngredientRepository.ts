...
import { RepositoryError } from '../../shared/types/Common';
...
export class PrismaIngredientRepository implements IIngredientRepository {
  ...
  async findAll(): Promise<IngredientWithRelations[]> {
    try {
      ...
      return ingredients.map(ingredient => this.mapToIngredientWithRelations(ingredient));
    } catch (error) {
   ...
   * 材料を更新
   */
  async update(id: bigint, ingredientData: Partial<Ingredient>): Promise<Ingredient> {
    try {
      const ingredient = await prisma.ingredient.update({
        ...
        data: {
          ...
          ...(ingredientData.originalName && { originalName: ingredientData.originalName }),
          updatedAt: new Date()
        }
      });
  ...
  async count(): Promise<number> {
    try {
      return await prisma.ingredient.count();
    } catch (error) {
   ...
   */
  private mapToIngredientWithRelations(prismaIngredient: any): IngredientWithRelations {
    const baseIngredient = this.mapToIngredient(prismaIngredient);
    return {
      ...baseIngredient,
      // ingredientNutrientsから栄養素情報を抽出
      nutrients: prismaIngredient.ingredientNutrients?.map((inNutrient: any) => ({
      ...
      ingredientNutrients: prismaIngredient.ingredientNutrients?.map((inNutrient: any) => ({
        id: inNutrient.id,

...
import { INutrientRepository } from '../../shared/interfaces/INutrientRepository';
import { 
  Nutrient, 
  CreateNutrientInput, 
  UpdateNutrientInput 
} from '../../shared/types/Nutrient';
...
export class PrismaNutrientRepository implements INutrientRepository {
  ...
  async count(): Promise<number> {
    try {
      return await prisma.nutrient.count();
    } catch (error) {
  ...
  private mapToNutrient(prismaNutrient: any): Nutrient {
    return {
      ...
      parentId: prismaNutrient.parentId ? prismaNutrient.parentId.toString() : null,
